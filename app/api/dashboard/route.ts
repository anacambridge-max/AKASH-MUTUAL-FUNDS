...
function dateKey(v:string){const p=v.split('-').map(Number);return p.length===3?`${p[2].toString().padStart(4,'0')}-${p[1].toString().padStart(2,'0')}-${p[0].toString().padStart(2,'0')}`:v;}
...
async function analytics(history:Point[],nav:number|null,mappedNames:string[],indexHistories:Map<string,Point[]>):Promise<Analytics>{
 const fr=returns(history);const negative=[...fr.values()].filter(x=>x<0).map(Math.abs);const current=history.length>1?((history.at(-1)!.value/history.at(-2)!.value)-1)*100:null;
 const cheapness=current!=null&&current<0?Math.min(100,percentile(Math.abs(current),negative)*1.05):0;
 ...
 const b=beta(fundArr,basketArr);const basketMoveNow=returnMaps.map(m=>m.get([...m.keys()].at(-1)??'')).filter((x):x is number=>x!=null);const est=b!=null&&basketMoveNow.length?b*basketMoveNow.reduce((a,x)=>a+x,0)/basketMoveNow.length:null;
 return{beta:b,volatility:vol,drawdown:dd,cheapness,historyCoverage:Math.min(100,history.length/500*100),basketSensitivity:b,estimatedChange:est};
}
...
export async function GET(){
 try{
  ...
  const output=await Promise.all(funds.map(async([code,name,category,mappedNames])=>{
   ...
   const an=await analytics(h,nav,mappedNames,indexHistories);const relative=change!=null&&move!=null?change-move:null;
  ...
  return NextResponse.json({updatedAt:new Date().toISOString(),funds:output,indices,sectors,summary:{trackedFunds:funds.length,liveFunds:output.filter(x=>x.status==='LIVE').length,liveIndices,unavailableIndices:indices.length-liveIndices,fallingIndices:falling,buySignals:output.filter(x=>x.signal==='BUY'||x.signal==='STRONG BUY').length,accumulateSignals:output.filter(x=>x.signal==='ACCUMULATE').length,breadth:Number(breadth.toFixed(1)),regime,dataConfidence:overallDataConfidence,coverage:`${liveIndices}/${indices.length}`},disclaimer:'Mapped indices are analytical proxy baskets, not actual portfolio holdings. Beta is estimated from fund NAV history versus available mapped proxy-index history.'},{headers:{'Cache-Control':'no-store'}});
