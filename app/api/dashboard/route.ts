...
const funds: FundConfig[] = [
 ...
 ['120821','Quant Multi Asset Fund Direct Growth','Multi Asset',['NIFTY 50','NIFTY BANK']],
 ...
 ['150714','UTI Gold ETF FoF Direct Growth','Gold',['NIFTY 50']],
...
async function fund(code:string):Promise<FundSnapshot>{
 try {
  ...
  return {nav:Number.isFinite(nav)?nav:null,previousNav:Number.isFinite(previousNav)?previousNav:null,change:Number.isFinite(nav)&&Number.isFinite(previousNav)&&previousNav!==0?((nav-previousNav)/previousNav)*100:null,date:d[0]?.date ?? null,status:Number.isFinite(nav)?'LIVE':'UNAVAILABLE'};
 } catch { return {nav:null,previousNav:null,change:null,date:null,status:'UNAVAILABLE'}; }
}

async function nseIndex(name:string):Promise<IndexSnapshot>{
 try {
  const j = await json('https://www.nseindia.com/api/allIndices',{'accept-language':'en-US,en;q=0.9','referer':'https://www.nseindia.com/market-data/live-market-indices','sec-fetch-site':'same-origin'});
  const rows = Array.isArray(j.data)?j.data:[];
  const aliases:Record<string,string[]>={
   'NIFTY NEXT 50':['NIFTY NEXT 50','NIFTY NEXT 50 TRI'],
   'NIFTY HEALTHCARE':['NIFTY HEALTHCARE','NIFTY HEALTHCARE INDEX'],
   'NIFTY SERVICES':['NIFTY SERVICES','NIFTY SERVICES SECTOR']
  };
  const wanted=aliases[name]||[name];
  const row=rows.find((x:any)=>wanted.includes(String(x.index||'').trim().toUpperCase()));
  if(row&&Number.isFinite(Number(row.last))) return {name,value:Number(row.last),change:Number(row.percentChange)||0,status:'LIVE'};
  throw new Error('Index not found');
 } catch { return yahooIndex(name); }
}

const yahooSymbols:Record<string,string>={'NIFTY 50':'^NSEI','NIFTY NEXT 50':'^NSMIDCP','NIFTY BANK':'^NSEBANK','NIFTY IT':'^CNXIT','NIFTY INFRA':'^CNXINFRA','NIFTY SERVICES':'^CNXSERVICE'};
...
async function yahooIndex(name:string):Promise<IndexSnapshot>{
 ...
 try {
  ...
  const last=Number(c.at(-1)),prev=Number(c.at(-2));
...
export async function GET(){
 const [fundData,indexData]=await Promise.all([Promise.all(funds.map(async ([code,name,category,sectors])=>({code,name,category,sectors,data:await fund(code)}))),Promise.all(indexNames.map(n=>nseIndex(n)))]);
 ...
 const enriched=fundData.map(f=>{
  ...
  const correction=Math.max(0,-navChange),marketWeakness=Math.max(0,-sectorMove);
