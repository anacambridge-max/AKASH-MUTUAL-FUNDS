...
export async function GET(){
 try{
  ...
  const output=await Promise.all(funds.map(async([code,name,category,mappedNames])=>{
   ...
   const base=prev??(nav!=null&&change!=null?nav/(1+change/100):null);const estimatedChange=move!=null&&an.beta!=null?move*an.beta:null;const estimatedNav=base!=null&&estimatedChange!=null?base*(1+estimatedChange/100):null;
